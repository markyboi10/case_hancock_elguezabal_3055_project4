...
export async function POST(req: NextRequest) {
    ...
    if (typeof valueToJson === 'string' && valueToJson.toLowerCase() === 'all') {
        ...
        try {
            const randomArtworksResponse = await fetch(`http://cs.merrimack.edu:8000/api/randomartworksall/`, {
        ...
        try {
            ...
            const randomArtworksResponse = await fetch(`http://cs.merrimack.edu:8000/api/randomartworksint/`, {
                method: 'POST', // Type post
                headers: {
                    'Content-Type': 'application/json',
                    'Cache-Control': 'no-cache, no-store, max-age=0, must-revalidate',
